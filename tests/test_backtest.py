...
def make_store(paths: dict[str, list[tuple[float, float, float, float]]],
               start: str = "2024-01-02") -> HistoricalBarStore:
...
def test_snapshot_asof_ignores_future_bars() -> None:
    ...
    assert (snap1.current_price, snap1.atr_14, snap1.volume_ratio) == (
        snap2.current_price, snap2.atr_14, snap2.volume_ratio
    )
...
def _engine_with_open_position(store: HistoricalBarStore) -> tuple[BacktestEngine, str]:
    cfg = BacktestConfig(universe=["AAA"], start="x", end="y", starting_capital=1000.0,
                         slippage_per_side=0.0005, db_path=":memory:")
    eng = BacktestEngine(cfg, store=store)
    eng.portfolio.on_buy(100.0)
    trade_id = eng.db.insert_trade_entry(
        symbol="AAA", entry_price=100.0, position_size_usd=100.0, shares=1.0,
        contributing_agents={}, aggregated_confidence=0.8, account_equity_at_entry=1000.0,
        atr_pct_at_entry=0.02, market_regime_at_entry="normal", stop_loss_pct=HARD_STOP_LOSS_PCT,
        take_profit_pct=DEFAULT_STRATEGY.take_profit_pct, config_version_id=None,
        active_model="claude-fable-5", entry_ts="2024-01-02T20:00:00+00:00",
    )
    return eng, trade_id
...
def test_entry_price_includes_buy_slippage() -> None:
    ...
    cfg = BacktestConfig(universe=["AAA"], start="x", end="y", slippage_per_side=0.001,
                         db_path=":memory:")
    eng = BacktestEngine(cfg, store=store)
...
def _closes_with_thesis(thesis: bool) -> list[tuple[str, str, float]]:
    ...
    cfg = BacktestConfig(universe=["AAA"], start="x", end="y", starting_capital=1000.0,
                         db_path=":memory:", thesis_enabled=thesis)
    eng = BacktestEngine(cfg, store=store)
    eng.portfolio.on_buy(100.0)
    ...
    return [(str(t["symbol"]), str(t["exit_reason"]), round(float(t["realized_pnl"]), 4))
